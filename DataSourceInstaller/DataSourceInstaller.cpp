...
#define EXECUTABLE "DataSourceService.exe"
#define SERVICE_NAME "VehicleLocationDataSource"
...
int __cdecl wmain(void)
{
	...
	printf("===================================\n");
	printf("Enter Listen Port (example: 5353): ");
	...
	iResult = RegSetValueEx(hKey, L"ListenPort", 0, REG_DWORD, (LPBYTE)&w_portno, sizeof(w_portno));
	if (iResult != ERROR_SUCCESS) {
		fprintf(stderr, "Failed to create registry key ListenPort [errno %d]\n", iResult);
		goto Exit;
	}
